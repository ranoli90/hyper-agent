...
test.describe('HyperAgent Extension - Basic Functionality', () => {
  ...
  test('should have proper meta and structure', async ({ page }) => {
    ...
    // Check charset (use toBeAttached instead of toBeVisible for meta tags)
  ...
  test('should handle keyboard events', async ({ page }) => {
    ...
    // Get the actual value and check it contains 'test'
    const value = await commandInput.inputValue();
    expect(value.trim()).toContain('test');

    // Test Enter key
    await page.keyboard.press('Enter');

    // Test backspace
    await commandInput.fill('tes');
